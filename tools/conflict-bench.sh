...
for cand in \
    ...
    "$target_dir/debug/zai" "$target_dir/debug/zai.exe"; do
    [ -n "$cand" ] || continue
    if zai_capable "$cand"; then
        zai=$cand
        break
    fi
done
...
if [ -z "$zai" ]; then
    echo "== ファイル所有リース (zai lease) を持つ zai が見つかりません。ビルドします:" >&2
    echo "   cargo build --release --bin zai" >&2
    (cd "$root" && cargo build --release --bin zai)
    for cand in "$target_dir/release/zai" "$target_dir/release/zai.exe"; do
        if zai_capable "$cand"; then
            zai=$cand
            break
        fi
