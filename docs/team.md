...
* **完了報告はファイルで受ける** (`src/team/outbox.rs` が取り決めの
  ...
  **読んで解析して配送できたときだけ消す**。読めないファイルは
  `MAX_ATTEMPTS` 回まで読み直し、それでも駄目なら `rejected/` へ隔離する
...
* **意味として却下された報告は消さずに `rejected/` へ隔離する。** 隔離先を
  作れなければ同じ場所で拡張子を外し、それも駄目ならそのまま残して読み直しを
  止める。**どの道でもファイルは消さない** — 却下された報告は「何を書いたのか」
