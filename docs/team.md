...
直接依頼の報告先は、開いたフォルダの `.zai-team-worktrees/.control/outbox/<run-id>/` に置きます。成果物とは分離した内部領域で、担当範囲の計測や最終納品から除外します。旧Runの報告先は維持します。画面からの代替読取りも、直近のスクロールバックとJSONの括弧を保持して行います。

GUI の直接依頼によるチーム開発では、開いたフォルダ直下の `output/` に本体と必要な同梱物をまとめます。仕様の相対ファイル名は `output/` 配下として扱い、既存プロジェクトを利用する場合も必要なファイルをここへ揃えます。仕様生成時に納品ルート、ファイル構成、利用入口、接続契約、全体動作の確認手順を揃えます。

`part-N` は内部の隔離作業先です。担当別の完成品を納品するためのフォルダではありません。各担当は共通構成の担当部分を実装し、最後の統合担当が差分を取り込み、不足を補い、納品先の入口から一連の利用操作を確認します。最終報告には納品先・入口・利用手順・検証結果・未確認事項を含めます。最終反映では同じチームの全担当の `output/` の差分を自動収集し、統合担当の修正と合わせて、開いたフォルダ直下の `output/` へ保存します。`part-N` の階層は納品先へ持ち込みません。開始時点から変更のないファイルは他担当の変更を上書きしません。同じパスへの異なる変更やファイル・フォルダの衝突は統合担当へ差し戻し、統合担当が変更した内容を採用します。不要な担当ファイルの除外や開始時点への復元は、統合担当の完了報告の `excluded_files` でファイル単位に明示できます。除外専用のファイルは作成しません。元フォルダへの反映には既存の競合検出を使用します。

納品先には仕様で必要な本体・設定・同梱物だけを保存し、作業メモ・自己評価JSON・進捗ログ・重複した完了報告・空の雛形を追加しません。実行設定や要求されたデータとして必要なJSONは保存します。チーム制御の報告は専用outboxへ送り、納品先へ複製しません。既存の利用者ファイルや必要な検証資料は削除しません。

最終反映の前に、凍結した実ファイルを検査します。`output/` 外の変更・削除、または `output/` 内に通常ファイルがない候補は反映せず、統合担当に修正を差し戻します。再試行には既存の上限を適用します。内部の隔離コピー・再開や競合復旧に必要な制御記録は納品物に含めません。

保存先は実際の反映処理でも検査します。あらゆる形式の成果物の動作を自動保証するものではなく、実施できなかった確認は未確認として報告します。

**GUI の直接依頼は、実装前に軽量な並列仕様を一度だけ生成する。**
原依頼の全文を保持したまま、担当・担当ファイル・接続契約・依存・完了条件を
決め、独立した担当から同時に実装する。エージェント数は同時実行数であり、
仕様の担当数を切り捨てる上限ではない。仕様作成後に企画・設計タスクを
重ねて追加しない。書式と品質保持の条件は
[並列開発仕様](team-parallel-spec.md)を参照。

以下は主に SPEC ファイルを渡す従来経路と共通 Runtime の説明。
GUI の自然文から仕様を生成する処理と、その仕様を Task Graph へ変換する
決定的な `StaticPlanner` は別の処理である。
...
* **利用者の index を書き換えない。** `IndexGuard` が呼び出し前の中身を覚え、
  どのエラー経路でも Drop で戻す。基準点を作ると index を HEAD へ揃える
  ことになるので、**揃えても何も失われないときだけ**進める (index が空か、
  新しい木と完全に一致するとき)。一部だけ stage した・`add -N` した・削除や
  rename を stage した状態では**コミットを作らずに断る** — 人が自分で
  コミットするか staging を畳めば、次に押したときは通る
* linked worktree では `git rev-parse --absolute-git-dir` が返すほうを使う
  (`.git` はファイルなので `ws/.git/index` を決め打つと別のリポジトリを触る)
...
~/.zaivern/team/<ワークスペースキー>/
  ...
  runs/<run_id>/   ← Run ごとの保存 (旧版の複数Runも壊さず保持)
  closed/<run_id>  ← 閉じた Run の墓標 (削除の前に書く。後始末が済んだら消える)
  outbox/<run_id>/ ← 完了報告の置き場 (Run ごと。閉じると消える)
    <agent-id>-<一意な値>.json.tmp  ← 書きかけ (読まない)
    <agent-id>-<一意な値>.json      ← rename で公開された正式な報告
    rejected/                       ← 取り込めなかった報告 (理由は時系列に残る)
```

* **完了報告はファイルで受ける** (`src/team/outbox.rs` が取り決めの
  置き場)。画面のグリッドはカーソル移動で描く CLI で行が潰れるため。
  エージェントは `.json.tmp` へ書き切ってから同じフォルダ内で rename する
  (原子的なので書きかけを読まれない)。読む側は `.json` だけを見て、
  **読んで解析して配送できたときだけ消す**。読む前に metadata と通常
  ファイルであることを確認し、再確認後も上限+1バイトで読み止める。巨大、
  非UTF-8は本文全体を読まずに`rejected/`へ隔離する。symlink、ディレクトリ、
  特殊ファイルは走査対象にせず、走査後の差し替えもリンク先を読まず拒否する。
  一時的な読込失敗や書きかけJSONだけを`MAX_ATTEMPTS`回まで読み直す
* 担当の照合は**ファイル名と本文の両方** — ファイル名は `<agent-id>` の
  直後が `-` のものだけ (`agent-1` は `agent-10-…` に当たらない)、本文の
  `agent_id` がそれと一致しなければ配送しない
* 置き場と配送cursorは Run ごと。走査は前回のRunの次からround-robinで
  1 Run 1ファイルずつ進み、全体の1 tick上限は維持する。旧版から複数Runを
  復元した内部状態でも、先頭Runの大量・破損ファイルが後続を飢餓させない
* **4 種類とも置き場が正規の経路。** 完了報告だけを置き場へ移しても
  ...
  一意に決まるときだけ受ける
* **重複判定は本文だけで数えない。** 送り主・種別・対象タスク・本文の指紋
  (128bit + 長さ) を鍵にする。本文だけにすると、別のレビュー担当が同じ文面
  (`{"verdict":"approve"}` のように送り主も対象も本文に無い形) を返しただけで
  2 通目が消え、タスクが `Reviewing` のまま止まる
* **却下されたものは重複台帳へ入れない。** 入れると、直して出し直した報告まで
  「もう見た」で捨てることになる。台帳へ入れるのは適用が済んでから
* **意味として却下された報告は消さずに `rejected/` へ隔離する。** 同名の
  証拠が既にあれば一意な名前へ移し、先の証拠を上書きしない。隔離先を
  作れなければ同じ場所で拡張子を外し、それも駄目ならそのまま残して読み直しを
  止める。**どの道でも証拠を失わない** — 却下された報告は「何を書いたのか」
  の唯一の証拠なので、消すと直しようが無くなる
...
* **Run を閉じる (✕) と、結び付いている全セッションの停止が直接出る**
  ...
  残る。停止は Run を記憶から外した後に実行側が取り出すので、
  `take_stops` だけは生存 Run の選り分け (`mine`) を通さない
* **閉じた Run は次の起動で復活しない。** 保存 (`runs/<run_id>/`) を消す前に
  墓標 (`closed/<run_id>`) を原子的に書き、復元は墓標を先に見る。削除に
  失敗したら帯に「保存」と「置き場」を区別して出し、次の起動が片付けを
  試し直す。全部片付いたら墓標も消える。`run_id` はフォルダ名の関門
  (`outbox::safe_child`) を通したものしか保存にも削除にも使わない
...
* 画面走査は400ms間隔。正規経路はbounded readされたoutboxで、画面は
  互換用の控えとして上限内の末尾だけを読む
