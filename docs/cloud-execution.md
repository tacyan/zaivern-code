...
26 台目以降が Registry に入らず**、Scheduler からは存在しないことになる。
台数を増やしたときにだけ静かに取りこぼすので、少ない台数の試験では
永久に見つからない。

`get_all_pages` が `meta.pagination.next_page` に従って辿る
(`/servers` / `/server_types` / `/locations` の 3 つとも)。止まる理由は 3 つ:

1. `next_page` が無い / null (**正規の終わり方**)
2. 次の番号が**進んでいない** (壊れた応答で同じページを回り続けない)
3. `MAX_PAGES` (100 ページ = 5000 件) に達した

**ページ番号は推測で増やさない** — 次があると相手が言ったときだけ進む。
途中のページが失敗したら、半端な一覧を返さずに失敗として返す
(「一部だけ取れた」を成功にすると、消えた実行先と区別が付かない)。

**ページ送りは Provider の中だけに閉じる。** Scheduler も Registry も
Hetzner のページ付けを知らない (知ると、次の Provider で同じ形が使えなくなる)。

### 失敗の応答の本文を捨てない

`ureq` は既定で 4xx / 5xx を `Error::StatusCode(code)` にするが、**その形には
本文が入っていない**。Hetzner は失敗の理由を `{"error":{"message":…}}` で
返すので、既定のままだと「401 でした」までしか言えない。しかも偽 HTTP の
試験では本文を渡せるため、**試験は緑のまま実運用でだけ診断情報が消える**。

`http_status_as_error(false)` にして、状態コードの解釈は `classify`
(Provider) の仕事に戻した。本文の読み取りには上限があり (4 MiB)、読めなかった
ときも**状態コードは失わない** — ただし成功した応答が読めなかったときだけは
失敗として返す (空の本文を成功として渡すと「空の一覧」に見える)。

`Agent` は使い回す (接続の池を共有し、TLS の握手をやり直さない)。待ち時間は
要求ごとに上書きするので、使い回しても 1 本ごとの上限は変わらない。
