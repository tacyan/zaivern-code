...
///
/// **コミットを渡さない**ので、どちらの git でも実マージは起きず usage が出て
/// 終わる。オブジェクト DB にも作業ツリーにも 1 バイトも書かない。
...
/// git 自身が出す usage を読む。
///
/// ## 出力の見分け方 (手元の git 2.47.1 と 2.38 未満の実装から)
///
/// | git | 出力 |
/// |---|---|
/// | 2.38+ | `usage: git merge-tree [--write-tree] …` — 選択肢一覧に `--write-tree` が載る |
/// | 2.38 未満 | `usage: git merge-tree <base-tree> <branch1> <branch2>` — `--write-tree` が出ない |
/// | option を解析する版 | ``error: unknown option `write-tree'`` |
///
...
/// `fatal: …` だけが出てどちらの条件も満たさず false になる (fail-closed)。
pub fn probe_supports_merge_tree(stdout: &str, stderr: &str, code: Option<i32>) -> bool {
    // コミットを渡さずに **成功** する git は存在しない。0 が返ったなら我々の
    // 想定したコマンドではないので、使えると見なさない (「推測しない」)。
    if code == Some(0) {
        return false;
    }
...
";
    ...
    fn mergetreeの能力はバージョンではなく叩いて確かめる() {
        // 2.38+ がコミット無しで出す usage (実測。**stderr 側**に出る)。
        ...
        // option を解析する古い版は unknown option と言う。
        ...
        // 「usage に載っているが unknown option とも言われた」矛盾は
        // 使えない側へ倒す (推測しない)。
        ...
        // コミット無しで成功する git は存在しない = 想定したコマンドではない。
    ...
    ///
    /// 番号での判定はバックポート版・機能削除版を必ず取り違えるので、
    /// うっかり戻ってしまわないよう構造で縛る。
    #[test]
    ...
    fn 離れた変更はコミット済みならgitの判定で情報へ落ちる() {
        ...
        let probed = run_probe(&base, &merge_tree_probe_argv())
            .is_some_and(|(so, se, code)| probe_supports_merge_tree(&so, &se, code));
        if probed {
