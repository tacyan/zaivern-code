...
pub(crate) mod bundled_pack {
    ...
    /// 同梱パックが syntect の**既定**構文を奪わないこと。
    ///
    /// 出荷時に実際に使うのは two-face の拡張セット (220 構文) で、
    /// そちらと重なる拡張子は `highlight::Highlighter::lang_for` が
    /// **syntect 側を優先する**ので、ここでの重なりは問題にならない
    /// (経路そのものは `highlight::pack_integration` が固定している)。
    /// ここは「素の syntect でも破綻しない」ことの番人として残す。
