...
### 💡 スーパーエージェント — 見張りに、頭脳を足す
監視そのものは Rust のルールで動いている。停滞やループを見つけるのに LLM は要らないし、**見張り役が LLM だと、見張り役が壊れたときに誰も気づけない**。だから検知は決定的なコードに任せてある。

そのうえで「で、どうするのが正解か」の判断だけを、AI に相談できる。コックピットの「💡 スーパーエージェント」で、**どの CLI エージェントに監督させるかを選ぶ**だけ。

- 既定は **「なし」**。選ばなければ LLM には一切問い合わせない
- 選べるのは**非対話実行に対応したエージェントだけ**。対応していないものを選ぶと対話画面が開いたまま永久に返ってこないので、そもそも選択肢に出さない
- 渡すのは**伏字処理した直近の出力だけ**。API キー・GitHub トークン・メールアドレス・ホームディレクトリのパスは自動で伏せる
- **監督役も、ほかと同じように監視される**。特別扱いすると、そこが単一障害点になる
- 監督役は**普通の作業にも使える**。監視専用に一枠を潰す必要はない。ただし自分自身の停滞を自分に診断させることはしない(止まっている相手に「なぜ止まったの」と聞いても返ってこない)
- **AI の助言でも、権限モードのゲートは変わらない**。再起動や停止を勧めてきても、⚡全自動モードであっても確認を挟む
- 返答が解釈できなければ**何もしない**。曖昧な答えから行動をひねり出す見張り役は、黙っている見張り役より危ない
...
├── diagnostician.rs 監視役 LLM(選んだ CLI エージェントを非対話で呼んで診断させる)
