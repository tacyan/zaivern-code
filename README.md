...
### 💬 差分へのインラインレビューコメント
PR の差分タブでも、レースの差分タブでも、**行をクリックしてその場にコメントを書けます**。

- コメントは「ファイル + 旧/新 + 行番号」に紐づくので、差分を読み直しても、スクロールで仮想化されても位置を見失わない
- スレッドごとに `解決` / `未解決に戻す` / `編集` / `削除`。ツールバーには「レビューコメント n 件 (未解決 m 件)」
- 未解決のコメントは**まとめて 1 通のプロンプトになる**。`以下のレビューコメントに対応してください:` に続けて `@パス:行`(削除行には `(削除行)`)・引用・本文が、ファイル順 → 行順で並ぶ。`コピー` でエージェントの入力欄へそのまま渡せる。**解決済みは自動的に外れる**ので、直し終わった指摘を送り直すことはない
...
- [ ] プロジェクト全体検索の強化 — 正規表現・グロブ・単語単位・ドライラン付き一括置換(基盤は実装済み、UI 配線は今後)
- [ ] エージェント横断の使用量集計と枯渇予測(オフラインで完結する基盤は実装済み、パネル表示は今後)
- [ ] 過去セッションの一覧と再開(claude / codex のセッション記録の読み取りは実装済み、UI は今後)
- [ ] 改行コードの検出・変換 + 保存時クリーンアップ(判定と変換の純関数は実装済み、保存経路への配線は今後)
