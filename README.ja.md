...
遅くなります。しかも規模が上がるほど効きます（ゲートが書き込みの経路に居るため）。実測は
4〜8 体で p50 40〜50ms、64 体で p50 298.7ms。さらに 32 体以上では、時間内に判定しきれない
ときに `busy-deny` が出ます。推測せずに断る側へ倒した結果で、再試行すれば通りますが、
利用者からは「たまに拒否される」として見えます。1〜2 体なら、ゲートは臨界路に居ません。
