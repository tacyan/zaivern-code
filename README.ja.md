...
Wi-Fi 内での利用で、そうでない場合は 2 つの経路が使えます。**Tailscale** (PC と
スマホが同じ tailnet に居るだけでよく、踏み台は要りません) と、既に SSH で入れる
ホストを中継する SSH トンネルです。経路を変えても**変わるのは待ち受け先だけ**で、
トークンもポートもページも同じなので、既に読み取った QR はそのまま使えます。

Tailscale モードは tailnet のアドレスと `127.0.0.1` **だけ**で待ち受けます。
喫茶店や空港の Wi-Fi に居ても、その LAN からはポートが見えません。
tailnet のアドレスはカーネルの経路表から引いており、`tailscale` コマンドは
実行しません — macOS のそれはデーモンに繋がらないと**返ってこない**シェル
ラッパで、ぶら下がった子は UI を固めるためです。
